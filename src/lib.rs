...
pub fn load_wavefront(
    ...
    data: &[u8],
    texture: &[u8],
) -> (glium::vertex::VertexBufferAny, glium::Texture2d) {
    #[derive(Copy, Clone)]
    struct Vertex {
        position: [f32; 3],
        normal: [f32; 3],
    ...
    let texture = image::load(std::io::Cursor::new(texture), image::ImageFormat::Jpeg)
        .expect("failed to load texture")
        .to_rgba8();
    let texture_dimentions = texture.dimensions();
    let texture =
        glium::texture::RawImage2d::from_raw_rgba_reversed(&texture.into_raw(), texture_dimentions);
    ...
    let vert_buff = glium::vertex::VertexBuffer::new(display, &vertex_data)
        .unwrap()
        .into();
    let  diffuse_texture = glium::texture::Texture2d::new(display, texture)
        .expect("failed to transform raw image to glium struct");
    (vert_buff, diffuse_texture)
}
...
impl<T: ApplicationContext + 'static> State<T> {
    pub fn new(event_loop: &glium::winit::event_loop::ActiveEventLoop, visible: bool) -> Self {
        let window_attributes = glium::winit::window::Window::default_attributes()
            .with_title(T::WINDOW_TITLE)
            .with_visible(visible);
        let config_template_builder = glium::glutin::config::ConfigTemplateBuilder::new();
        let display_builder =
        ...
        let context_attributes = glutin::glutin::context::ContextAttributesBuilder::new()
            .build(Some(window_handle.into()));
        let fallback_context_attributes = glutin::glutin::context::ContextAttributesBuilder::new()
            .with_context_api(glutin::glutin::context::ContextApi::Gles(None))
            .build(Some(window_handle.into()));
        ...
        let attrs = glutin::glutin::surface::SurfaceAttributesBuilder::<WindowSurface>::new()
            .build(
                window_handle.into(),
